...
labels = ['T14', 'b', 't0mos', 't0spitz'] + 2*3*['LD'] + Nlightcurves*['RpRs'] + \
...
############################################################
### Init param procedure for phot_fit-tbMpinkSwhite.py
...
#scattercoeffs = np.array([0.002, 0.3, 0.0005, 0.0005, 0.1, 0.1, 0.05, 0.5, 
#                       0.01, 0.5] + Nlightcurves*[0.003] + Nlightcurves*[0.0003] + 
#                      Nbins*[0.7] + Nlightcurves*[0.15] + Nbins*[1.5] + Nbins*[0.04])
...
#spitzwhitekernelall_params = np.load(scratchpath+'max_lnp_params_201503040921.npy') 
#mossqexpcos_params = np.load(scratchpath+'max_lnp_params_mossqexpcos.npy')
#
## Combine these old best-fits, using white kernel hyperparams from the sqexpcos fit
#
#initT14, initb = aRsi2T14b(period, spitzwhitekernelall_params[0], spitzwhitekernelall_params[1])
#
#
#initP = np.concatenate([[initT14, initb],
#                        spitzwhitekernelall_params[2:-Nlightcurves], # all parameters except GP hyperparams
...
allchains = np.loadtxt(scratchpath + 'tbMpinkSwhite.dat')
...
for i in range(n):
    med, sig =  medsig(samples[:,i])
    initP[i] = med
    scattercoeffs[i] = 2.5*sig
...
nwalkersfactor = 4
nwalkers = nwalkersfactor*Nfreeparameters if nwalkersfactor*Nfreeparameters % nwalkersfactor == 0 else nwalkersfactor*Nfreeparameters + 1
...
if True:
    #Nhours = 48
    Nsteps = 1#1e6#int(Nhours*60*3.9) # 3.9 steps/min
    ...
    thisfilename = os.path.split(__file__)[-1].split('.py')[0]
