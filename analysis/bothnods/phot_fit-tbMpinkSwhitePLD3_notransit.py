...
paramlimits = Nlightcurves*[[-13, -6]] +\
               Nbins*[[-15, -5]] + Nbins*[[0, 0.5]] # GP hyperparams

labels = Nlightcurves*['GPw'] + Nbins*['GPamp'] + Nbins*['GPsig']
lastp = 0
...
def lnlike(theta, y_mos, yerr_mos, y_ch1, yerr_ch1, y_ch2, yerr_ch2):
    ...
    amp = np.exp(theta[Nlightcurves:Nlightcurves+Nbins])
    sig = theta[Nlightcurves+2*Nbins:]
    lnlikelihoodsum = 0.0
    ...
    for i in range(Nlightcurves):
        ...
        # For Spitzer light curves:
            ...
            lnlikelihoodsum += gp_objs[i].lnlikelihood(y_ch1)
        elif i == 9:
            gp_objs[i].kernel.pars = [w[i]]
            lnlikelihoodsum += gp_objs[i].lnlikelihood(y_ch2)
...
for i in range(len(lastpos)):
    ...
    #while proposedstep[]
    while not ((proposedstep[:Nbins] > paramlimits[Nlightcurves][0]).all() and 
               (proposedstep[:Nbins] < paramlimits[Nlightcurves][1]).all() and 
               (proposedstep[Nbins:] > paramlimits[Nlightcurves+Nbins][0]).all() and 
               (proposedstep[Nbins:] < paramlimits[Nlightcurves+Nbins][1]).all()):
...
pos = np.hstack([whiteparams, redparams])

Nfreeparameters = len(pos[0])
...
if True:
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, threads=14, a=1.5,
                                    args=(MOSFIREresiduals, lightcurve_errors, ch1residuals, ch1['e'], ch2residuals, ch2['e']))
