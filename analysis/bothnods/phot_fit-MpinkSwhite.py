...
scattercoeffs = np.array([0.2, 0.1, 0.0005, 0.0005, 0.1, 0.1, 0.05, 0.5, 
...
if True:
    ...
    f = open(chainpath+"MpinkSwhite-parallel.dat", "w") #iterations=500 -> 42 MB for raw text
    f.write('#'+' '.join(labels)+'\n')
    f.close()
    for result in sampler.sample(p0, iterations=Nsteps, storechain=False):
        f = open(chainpath+"MpinkSwhite-parallel.dat", "a")
