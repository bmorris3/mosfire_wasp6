...
#rawch1 = np.genfromtxt('spitzer/thirdPLD/wasp6_channel1.ascii')
#rawch2 = np.genfromtxt('spitzer/thirdPLD/wasp6_channel2.ascii')
...
if gpon:
    if gpred: 
        paramlimits = [[0.07, 0.11],  #aRs
                       ...
                       Nredbins*[[-20, 20]] + Nredbins*[[0, 0.5]] # GP hyperparams
...
#    if exptime < 0.00011574074074074075 and airmassvector is None:
#        return reparameterized_lc(T, b, t0, q1, q2, RpOverRs, F0, eccentricity,
...
for i in range(Nlightcurves):
    ...
    if i < Nbins:
        kernellist.append(kernels.WhiteKernel(np.exp(-10.5)))# + 
                      #e-3*kernels.ExpSquaredKernel(120)*kernels.CosineKernel(cosineperiod))
...
def lnlike(theta, y_mos, yerr_mos, y_ch1, yerr_ch1, y_ch2, yerr_ch2):
    ...
    if not gpon:
        ...
        for i in range(Nlightcurves):
            # For MOSFIRE light curves:
            if i < Nbins:
                lnlikelihoodsum += -0.5*np.sum((y_mos[:,i] - mosfiremodel[:,i])**2/yerr_mos[:,i]**2)
            
            # For Spitzer light curves:
            elif i == 8:
                lnlikelihoodsum += -0.5*np.sum((ch1['f'] - ch1model)**2/yerr_ch1**2)
            elif i == 9:
                lnlikelihoodsum += -0.5*np.sum((ch2['f'] - ch2model)**2/yerr_ch2**2)
    else: 
        w = np.exp(theta[-Nlightcurves-2*Nbins:-2*Nbins])
        amp = np.exp(theta[-2*Nbins:-Nbins])
        sig = theta[-Nbins:]
        ...
        for i in range(Nlightcurves):
            ...
            if i < Nredbins:
                ...
                gpobj = george.GP(kernels.WhiteKernel(w[i]) + 
                    amp[i]*kernels.ExpSquaredKernel(sig[i])*kernels.CosineKernel(cosineperiod))
                gpobj.compute(times)
                lnlikelihoodsum += gpobj.lnlikelihood(multiplyresiduals(y_mos[:,i] - mosfiremodel[:,i]))
        
            if i >= Nredbins and i < 8:
                gpobj = george.GP(kernels.WhiteKernel(w[i]))
                gpobj.compute(times)
                lnlikelihoodsum += gpobj.lnlikelihood(multiplyresiduals(y_mos[:,i] - mosfiremodel[:,i]))                
            # For Spitzer light curves:
                ...
                gpobj = george.GP(kernels.WhiteKernel(w[i]))
                gpobj.compute(ch1['t'])
                lnlikelihoodsum += gpobj.lnlikelihood(multiplyresiduals(ch1['f'] - ch1model))
            elif i == 9:
                gpobj = george.GP(kernels.WhiteKernel(w[i]))
                gpobj.compute(ch2['t'])
                lnlikelihoodsum += gpobj.lnlikelihood(multiplyresiduals(ch2['f'] - ch2model))
...
#allchains = np.load('tbMpinkSwhite.npy')
...
#initP = np.load('tmpmaxlnp.npy')#np.zeros(n)
#scattercoeffs = np.zeros(n)
#for i, label in enumerate(labels):
#    med, sig =  medsig(samples[:,i])
    # Always put big scatter on LD coeffs
#    if label == 'LD':
#        initP[i] = 0.01
#        scattercoeffs[i] = 0.1
#    else:
#    initP[i] = med
#    scattercoeffs[i] = 0.2*sig
#initP = max_lnp_params
#scattercoeffs = np.zeros(n)
#for i, label in enumerate(labels):
#    med, sig =  medsig(samples[:,i])
#    scattercoeffs[i] = 0.5*sig
...
if getinitposition:
    ...
    lastpos_extracolumns = stats.getlastchainstate(path, 8)
    ...
    #        pos = np.hstack([lastpos, whiteparams, redparams])
            ## Load results from phot_fit-mossqexpcos.ipynb for red noise parameters
            mossqexpcos = np.load('max_lnp_params_mossqexpcos.npy')
            redparams = np.zeros((len(lastpos), len(mossqexpcos[Nbins:])-6))
            for i in range(len(lastpos)):
                proposedstep = -1e10*np.ones_like(mossqexpcos[Nbins:-6])
                #while proposedstep[]
                while not ((proposedstep[:Nredbins] > paramlimits[-2*Nredbins][0]).all() and 
                           (proposedstep[:Nredbins] < paramlimits[-2*Nredbins][1]).all() and 
                           (proposedstep[Nredbins:] > paramlimits[-Nredbins][0]).all() and 
                           (proposedstep[Nredbins:] < paramlimits[-Nredbins][1]).all()):
                    #proposedstep = mossqexpcos[Nbins:] + np.concatenate([0.7*np.random.randn(Nbins), 
                    firsthalf = mossqexpcos[Nbins:Nbins+Nredbins] + 1*np.random.randn(Nredbins)
                    secondhalf = mossqexpcos[2*Nbins:2*Nbins+Nredbins] + 0.1*np.random.randn(Nredbins)
                    #proposedstep = mossqexpcos[Nbins:] + np.concatenate([4*np.random.randn(Nbins), 
                    #                0.1*np.random.randn(Nbins)])
                    proposedstep = np.concatenate([firsthalf, secondhalf])
                redparams[i,:] = proposedstep
            redparams[:,:Nredbins] = np.log(multiplyresiduals(np.exp(redparams[:,:Nredbins])))
            pos = np.hstack([lastpos, whiteparams, redparams])
    else: 
        pos = lastpos
    ...
    from moar import stats
    path = '/astro/store/scratch/tmp/bmmorris/longchains/mosfirespitzer/'+\
            'phot_fit-tbMpinkSwhitePLD3_mult_corrected201503170936/chains.dat'
    pos = stats.getlastchainstate(path, 8)

Nfreeparameters = pos.shape[1]

ndim = Nfreeparameters
#nwalkersfactor = pos.shape[0]#8
nwalkers = pos.shape[0]#nwalkersfactor*Nfreeparameters if nwalkersfactor*Nfreeparameters % nwalkersfactor == 0 else nwalkersfactor*Nfreeparameters + 1
...
plotgp = True
if plotgp:
    customcm = plt.cm.autumn
    #max_lnp_ind = np.argmax(lnp)
    max_lnp_params = initP
    cmap = plt.cm.autumn
    ...
    for i, ch, model, Nphases, lam in zip(range(2), [ch2, ch1], [ch2model, ch1model], [-182, -180], [3.6, 4.5]):
        ...
        phase = (ch['t'] -initP[3])/period - Nphases
...
if True:
    ...
    pool = emcee.interruptible_pool.InterruptiblePool(processes = 10)
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, a=2, pool = pool,
                                    args=(lightcurve, lightcurve_errors, ch1['f'], 
                                          ch1['e'], ch2['f'], ch2['e']))
