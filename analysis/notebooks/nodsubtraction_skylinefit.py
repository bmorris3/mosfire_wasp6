...
targetbounds = [385, 445]
compbounds = [1390, 1460]
roughnodcentroids = [200, 600] # Rough indices between A and B nods
apertureradius = 28#20#8
bg_o = 5      # Background box outer limit
bg_i = 1.0#1.5#2.5    # Background box inner limit

#for i, imagepath in enumerate(wasp6paths_nodsub_odd[:1]):
for i, imagepath, imagesumpath in zip(range(len(wasp6paths_nodsub_odd)), wasp6paths_nodsub_odd, wasp6paths_sum_odd)[:1]:
    image = pyfits.getdata(imagepath)[:,paddingbounds[0]:paddingbounds[1]]
    skysum = np.mean([pyfits.getdata(sumpath) for sumpath in wasp6paths_sum_odd[i:i+10]],axis=0)[:,paddingbounds[0]:paddingbounds[1]]#skysum_fullframe[:,paddingbounds[0]:paddingbounds[1]]
    ...
    # Fit the sky lines to the background    
    skylinefitbounds = [1000, 1500]
    def fitfunc_clip(p):
        return p[0]*skysum[:,skylinefitbounds[0]:skylinefitbounds[1]].ravel() + p[1]
    def fitfunc(p):
        return p[0]*skysum + p[1]
    def errfunc(p):
        return fitfunc_clip(p) - gaussian_filter(image[:,skylinefitbounds[0]:skylinefitbounds[1]],2).ravel()
    initp = [-3e-3, 40]#[-1e-3, 0.0]
    bestp = optimize.leastsq(errfunc, initp)[0]
    #bestfitskylines = fitfunc(bestp).reshape(image[:,skylinefitbounds[0]:skylinefitbounds[1]].shape)
    bestfitskylines = fitfunc(bestp)
    print bestp

    fig, ax = plt.subplots()
    ax.plot(np.sum(gaussian_filter(image,2),axis=0), label='Image')
    #ax.plot(range(skylinefitbounds[0], skylinefitbounds[1]), np.sum(bestfitskylines,axis=0), label='Sum model')
    ax.plot(np.sum(bestfitskylines,axis=0), label='Sum model')
    ax.legend()
    #plt.imshow(np.sum(image,axis=1),interpolation='nearest',origin='lower')
    plt.show()  
