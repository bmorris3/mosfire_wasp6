...
wasp6paths_nodsub_odd = ["%s%04d%s" % ('/local/tmp/mosfire/2014sep18_analysis/m140918_', i,                                        'n.fits') for i in range(374,629,2)]
wasp6paths_sum_odd = ["%s%04d%s" % ('/local/tmp/mosfire/2014sep18_analysis/m140918_', i,                                        'sum.fits') for i in range(374,629,2)]
...
Nbins = 10
times = np.zeros(len(wasp6paths_nodsub_odd))
apertureradii = [28]#np.arange(30,45)#np.arange(10, 70, 5)#np.arange(14, 25, 2)
        ...
        for k in range(Nbins):
            ...
            for l, apertureradius in enumerate(apertureradii):
                ...
                if False:#j == 1:# and (i == 0 or i == 10):
                    ...
                    #plt.show()
                fluxes[i, j, k, l] = np.sum(withinaperture) - meanbackground*withinaperture.size
                errors[i, j, k, l] = np.sqrt(np.sum(np.sqrt(withinaperture_sum)))/np.sqrt(gain)# + meanbackground*withinaperture.size)
...
for l, apertureradius in enumerate(apertureradii):
    ...
    for eachbin in range(len(lightcurve[0,:])):
        ...
        bestp, bestfit = quicklsfit(eachbin)
        print 'bestp',bestp
    ...
    chisquared_allbins[l] = chi2(lightcurve_allbins, models_allbins, lightcurve_errors_allbins, 3*Nbins)
